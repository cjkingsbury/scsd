...
def uploaded_file():
    if request.method == 'POST':
        ...
        html = render_template('/nsd/nsd_html_template_v5.html',
                               ...
                               extras=nsd_object.extras(data['validtype'], data.get('rounding'), data.get('keith'),
                                                        cmap=data.get('cmap')))
...
def database_direct_ccdc(refcode):
    ...
    extras = '\n'.join([nsd_object.alert_section(),
                        render_template('/nsd/nsd_bdba_table_section.html',
                        ...
                        render_template('/nsd/nsd_mondrian_section.html',
...
def scsd_out():
    if request.method == 'POST':
        ...
        extras = scsd_obj.compare_table(data_path=dfs_path) + render_template('/scsd/scsd_hidden_raw_data_section.html',
        ...
        html = render_template(template,
                               ...
                               mondrian_fig=scsd_obj.mondrian(as_type='buffer',
                                                              cmap=data.get('cscheme_text', 'Spectral')),
...
def scsd_ccdc_random():
    ...
    scsd_obj.calc_scsd(False, bypass=True)
    if 'nearest' in dfrow.columns:
        extras = scsd_obj.compare_table(data_path=dfs_path, bypass=True, nearest_dict=dfrow['nearest'].values[0])
    else:
        extras = scsd_obj.compare_table(data_path=dfs_path)
    extras = extras + render_template('/scsd/scsd_hidden_raw_data_section.html', raw_data=scsd_obj.raw_data())
    data = {'model_name': model.name, 'refcode': refcode}
...
def scsd_mod_lookup(model_name):
    ...
    tstamp = str(date.today()).replace('-', '')
    extras = render_template('/scsd/scsd_hidden_raw_data_section.html',
