...
from xtab import *
import scsd, scsd_models, scsd_models_user

try: from flask_weasyprint import HTML, render_pdf, CSS
except: print('flask_weasyprint not available - pdf output disabled')

app = Flask(__name__, template_folder =  getcwd() + '/templates/',
           static_folder='static', static_url_path='')

u_folder = getcwd() + '/data/temp/'
dfs_path = getcwd() + '/data/scsd/'

@app.route('/nsd', methods = ['GET'])
def upload_file():
    return render_template('/nsd/nsd_uploader.html',
...
def uploaded_file():
    if request.method == 'POST':
        ...
        if data.get('IUPAC_numbering'):
            nsd_object = nsd_obj(filenm = u_folder + f.filename, calctype = 'correct_numbering')
        ...
        html = render_template('/nsd/nsd_html_template_v5.html',
                pdbpath = f.filename,
                time = ctime(time()),
                nsd = nsd_object.nsd_matrix, 
                skeletal_fig =  nsd_object.nsd_fig(),
                extras = nsd_object.extras(data['validtype'], data.get('rounding'), data.get('keith'), cmap = data.get('cmap')))
        
        if data['outtype'] == 'html':
            return html
        elif data['outtype'] == 'pdf':
            return render_pdf(HTML(string=html),
                download_filename = f.filename[:-4]+'_nsd.pdf')
...
def uploaded_file_ccdc():
    if request.method == 'POST':
        ...
        try: 
            ...
            nsd_object.calc_nsd()
        except IndexError: return fail_html
        ...
        if data['outtype'] == 'html':
            ...
            return render_pdf(HTML(string=html), download_filename = data.get('NAME_str').upper()+'_nsd.pdf')
            #return redirect(url_for('nsd'))
...
def database_direct_ccdc(refcode):
    ...
    extras = '\n'.join([nsd_object.alert_section(),
    render_template('/nsd/nsd_bdba_table_section.html', vals = nsd_object.bdba_extract_array(True, False)),
    render_template('/nsd/nsd_verbose_table_section.html', t = nsd_object.verbose_output()),
    render_template('/nsd/nsd_mondrian_section.html', mondrian_fig = nsd_object.mondrian(cmap = 'random'))
    ])
...
def scsd_out():
    if request.method == 'POST':
        ...
        model = scsd.model_objs_dict.get(data.get('model_name'),
            scsd_models_user.model_objs_dict.get(data.get('model_name'),None))
        
        if type(model) == scsd.scsd_model: ptgr, maxdist  = model.ptgr, model.maxdist
        else:                                     ptgr, maxdist  = data.get('point_group'), 2.2
        
        ats = scsd.import_pdb(u_folder + f.filename)
        scsd_obj = scsd.scsd_matrix(ats, model, ptgr)
        scsd_obj.calc_scsd(data.get('basinhopping') == 'True', by_graph = data.get('by_graph') == 'True')
        ...
        html = render_template(template,
            ...
            mondrian_fig = scsd_obj.mondrian(as_type = 'buffer', cmap = data.get('cscheme_text','Spectral')),
...
def scsd_ccdc_out(refcode):
    ...
    try: 
        df_name = combined_df[combined_df['name'].isin([refcode, refcode.upper()])]['df_name'].values[0]
    except IndexError: 
        return refcode + ' ' + refcode.upper() + ': Refcode not found in precomputed databases - use ./scsd'
    model = scsd.model_objs_dict.get(df_name,False)
    if isinstance(model, bool): return 'Model cannot be found for ' + refcode +' + '+ df_name
    try:  df = read_pickle(dfs_path + model.database_path)
    ...
    if 'nearest' in dfrow.columns: extras = scsd_obj.compare_table(data_path = dfs_path, 
        bypass = True, 
        nearest_dict = {x:sqrt(y) for x,y in dfrow['nearest'].values[0].items()})
    else: extras = scsd_obj.compare_table(data_path = dfs_path)
    extras = extras + render_template('/scsd/scsd_hidden_raw_data_section.html',raw_data = scsd_obj.raw_data())
    ...
    html = render_template(template,
        ...
        nsd_table = scsd_obj.html_table(n_modes = 2),
        mondrian_fig = scsd_obj.mondrian(as_type = 'buffer', cmap = random.choice(scsd.good_cmaps)),
...
def scsd_ccdc_random():
    ...
    scsd_obj = scsd.scsd_matrix(dfrow['coords'].values[0], model)
    scsd_obj.calc_scsd(False, bypass = True)
    if 'nearest' in dfrow.columns: extras = scsd_obj.compare_table(data_path = dfs_path, bypass = True, nearest_dict = dfrow['nearest'].values[0])
    else: extras = scsd_obj.compare_table(data_path = dfs_path)
    extras = extras + render_template('/scsd/scsd_hidden_raw_data_section.html',raw_data = scsd_obj.raw_data())
    data = {'model_name':model.name, 'refcode':refcode}
    template = '/scsd//scsd_html_template_v2.html'
...
def scsd_mod_out():
    if request.method == 'POST':
        ...
        model_ats = scsd.yield_model(model_ats_in, data.get('point_group'), bhopping = data.get('basinhopping'))
        model = scsd.scsd_model(model_name, model_ats, data.get('point_group'), 
        ...
        extras = render_template('/scsd/scsd_hidden_raw_data_section.html',raw_data = '\n'.join(for_mod_usr))

        #template = model_templates.get(data.get('model_name'),'/scsd_html_template_v2.html')
        html = render_template('/scsd/scsd_model_report.html',
            title = f.filename[:-4],
...
def scsd_mod_lookup(model_name):
    model = scsd.model_objs_dict.get(model_name,scsd_models_user.model_objs_dict.get(model_name,None))
    ...
    if model.database_path is not None:
        try:  
            ...
            extras = extras + '<br>' + ', '.join([link.format(x = refcode) for refcode in df['name'].values])
        except FileNotFoundError: 
            extras = extras + '<br> Database not on this server - contact Chris Kingsbury at ckingsbu@tcd.ie for data'
    html = render_template( '/scsd/scsd_model_report.html',
        title = model_name,
...
def scsd_mod_ext(model_name):
    ...
    if model.database_path is not None:
        try:  
            ...
            extras = extras + '<br>' + ', '.join(
                [coll.pca_kdeplot(x, as_type = 'html') for x in irs])
...
def scsd_data(model_name):
    model = scsd.model_objs_dict.get(model_name,scsd_models_user.model_objs_dict.get(model_name,None))
    try: return send_from_directory(dfs_path,model.database_path, as_attachment = True)  
    except FileNotFoundError: return 'Database not on this server - contact Chris Kingsbury at ckingsbu@tcd.ie for data'
        

@app.route('/scsd_models_table', methods = ['GET'])
def scsd_mod_all():

    model_names = list(scsd.model_objs_dict.keys()) + list(unique(list(scsd_models_user.model_objs_dict.keys())))
    table = [ '<th> name(link)  </th><th> chem formula </th><th>point group</th><th> data_path </th>']
    for name in model_names:
        model = scsd.model_objs_dict.get(name,scsd_models_user.model_objs_dict.get(name,None))

        table.append('</td><td>'.join([f'<td><a href=/scsd_model/{name}>{name}</a>', 
            model.chem_formula(), 
            model.ptgr_html, 
            str(model.database_path)+'</td>']))
    html = render_template( '/scsd/scsd_models_table.html',
        table = "<table id='modstab'><tr>" + '</tr>\n<tr>'.join(table) + '</tr></table>')
    
    return html

@app.route('/xtal_table', methods = ['GET'])
def xtab_in():
    return render_template('/scsd/xtal_table_uploader.html')

@app.route('/xtal_table_output', methods = ['GET','POST'])
def xtab_out():
    if request.method == 'POST':
        f = request.files['file']
        f.save(u_folder + f.filename)
        g = open(u_folder + f.filename)
        xdb_dict = cif_to_dict(g, str(f.filename))
        return render_template('/scsd/xtal_table_output.html', d = xdb_dict)
